    ...
    public class CoordinateDDM : CoordinateBase
    {
        ...
        public override string ToString(string format, IFormatProvider formatProvider)
        {
            ...
            switch (format.ToUpper())
            {
                case "":
                case "DDM":
                    sb.AppendFormat(fi, $"{Math.Abs(this.LatDegrees)}° {this.LatMinutes:0.0#####}\' {(this.LatDegrees < 0 ? "S" : "N")}");
                    sb.AppendFormat(fi, $" {Math.Abs(this.LonDegrees)}° {this.LonMinutes:0.0#####}\' {(this.LonDegrees < 0 ? "W" : "E")}");
                    break;
                default:
